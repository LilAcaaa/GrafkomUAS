...
export class Player {
  ...
  update(dt,kotak) {
    if (this.mesh && this.animations) {
      ...
      const previousPosition = this.mesh.position.clone(); // Save the previous position
      ...
      for (let i = 0; i < kotak.length; i++) {
        if (this.box.intersectsBox(kotak[i])) {
            this.mesh.position.copy(previousPosition); // Revert to previous position
            this.box.setFromObject(this.mesh); // Update bounding box again after reverting
...
export class PlayerController {
  ...
  onMouseWheel(event) {
    ...
    const newFocalLength = this.ThirdPersonCamera.camera.getFocalLength() + (event.deltaY < 0 ? 2 : -2);
    this.ThirdPersonCamera.camera.setFocalLength(Math.max(16.03, Math.min(75, newFocalLength)));
  }

  zoomIn() {
    const focalLength = this.ThirdPersonCamera.camera.getFocalLength();
    this.ThirdPersonCamera.camera.setFocalLength(focalLength + 5);
  }
...
export class ThirdPersonCamera {
  ...
  updateCameraPosition() {
    ...
    const y = this.positionObject.y + this.zoomLevel;
    const z = this.positionObject.z + this.zoomLevel;
  ...
  adjustZoom(deltaY) {
    const zoomSpeed = 0.1; 
    this.targetZoomLevel = THREE.MathUtils.clamp(this.targetZoomLevel + (deltaY * zoomSpeed), 0.5, 2);

    const minFocalLength = 10;
    const maxFocalLength = 30; 
    const focalLength = THREE.MathUtils.lerp(maxFocalLength, minFocalLength, this.targetZoomLevel);
